...
CONN_RATE_THRESHOLD=50         # 连接速率阈值（每秒新增连接数）
SYN_THRESHOLD=60               # SYN连接阈值
...
save_connection_history() {
    ...
    get_connection_stats | while read count ip; do
        # 为每个IP单独记录一行，格式为 "IP:计数"
        if [ ! -z "$ip" ]; then
            echo "IP:$ip:$count" >> "$HISTORY_FILE"
        fi
    done
...
calculate_connection_rate() {
    ...
    # 从备份的历史文件中获取上一次的数据
    if [ -f "${HISTORY_FILE}.old" ]; then
        last_timestamp=$(grep -m 1 "TIME:" "${HISTORY_FILE}.old" | cut -d':' -f2)
        last_ip_line=$(grep -m 1 "IP:$ip:" "${HISTORY_FILE}.old")
        if [ ! -z "$last_ip_line" ]; then
            last_count=$(echo "$last_ip_line" | cut -d':' -f3)
        fi
    fi
    
    # 如果没有上一次的记录，返回0
    if [ -z "$last_timestamp" ] || [ "$last_timestamp" -eq 0 ] || [ -z "$last_count" ]; then
        log_debug "IP $ip 没有历史记录，无法计算连接速率"
        echo "0"
        return
    fi
    ...
    # 记录详细的计算过程
    log_debug "IP $ip 的连接速率计算: 当前连接数=$current_count, 上次连接数=$last_count, 时间差=${time_diff}秒"
    
    # 计算连接速率（每秒新增连接数）
    local rate=$(( (current_count - last_count) / time_diff ))
    # 如果是负数，设置为0（可能是连接数减少了）
    if [ "$rate" -lt 0 ]; then
        rate=0
    fi
    
    log_debug "IP $ip 的连接速率计算结果: $rate/秒"
    echo "$rate"
}
...
process_connections() {
    ...
    # 检查SYN_RECV状态连接
    log_debug "检查SYN_RECV状态连接数 (阈值: $SYN_THRESHOLD)"
    head -n 20 "$syn_conn_file" | while read count ip; do
        if [ ! -z "$ip" ] && [ "$count" -ge "$SYN_THRESHOLD" ]; then
            log_debug "发现SYN连接过多IP: $ip (SYN连接数: $count, 阈值: $SYN_THRESHOLD)"
            if ! is_whitelisted "$ip" && ! is_blocked "$ip"; then
    ...
    log_debug "检查ESTABLISHED状态连接数 (阈值: $ESTABLISHED_THRESHOLD)"
    head -n 20 "$established_conn_file" | while read count ip; do
        if [ ! -z "$ip" ] && [ "$count" -ge "$ESTABLISHED_THRESHOLD" ]; then
            log_debug "发现ESTABLISHED连接过多IP: $ip (ESTABLISHED连接数: $count, 阈值: $ESTABLISHED_THRESHOLD)"
            if ! is_whitelisted "$ip" && ! is_blocked "$ip"; then
